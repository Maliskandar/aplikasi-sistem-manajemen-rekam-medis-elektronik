...
@section('content')
    <div class="container">
        ...
        <!-- Breadcrumbs End -->
        <h1 class="text-2xl font-bold mb-4 text-gray-700 dark:text-gray-400">Edit Kandang</h1>
        <form action="{{ route('admin.kandang.update', $kandang->id_kandang) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text"
                    ...
                    id="ukuran" name="ukuran" value="{{ $kandang->ukuran }}" required>
            </div>
            <div class="mb-3">
                ...
                <select
                    ...
                    <option value="1" {{ $kandang->ketersediaan ? 'selected' : '' }}>Tersedia</option>
                    <option value="0" {{ !$kandang->ketersediaan ? 'selected' : '' }}>Tidak Tersedia</option>
                </select>
            </div>
            <button type="submit"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 mt-2">Update</button>
        </form>
    </div>
@endsection
